...
public class KeeseStats : EnemyStats {
	...
	void OnTriggerEnter(Collider coll)
  {
    ...
    else if (coll.gameObject.tag == "Pit" || coll.gameObject.tag == "EnemyWall" || coll.gameObject.tag == "Wall" || coll.gameObject.tag == "Lock" || coll.gameObject.tag == "UpDoor" || coll.gameObject.tag == "RightDoor" || coll.gameObject.tag == "LeftDoor" || coll.gameObject.tag == "DownDoor")
    {
			GetComponent<Rigidbody>().velocity = Vector3.zero;
			alignWithGrid();
			nextBlock = Vector3.zero;
	...
	void chooseDest() {
		Vector3 roomPos;
		roomPos.x = Mathf.Floor(room.transform.position.x);
		roomPos.y = Mathf.Floor(room.transform.position.y);
		int minX = -5;
		int maxX = 6;
		int minY = -3;
		int maxY = 3;
